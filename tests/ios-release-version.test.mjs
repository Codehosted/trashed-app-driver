...
test('local and automatic iOS release builds use the same new store version', () => {
  ...
  assert.match(workflow, /marketing_version:[\s\S]*?default: "1\.0\.3"/);
  assert.match(workflow, /github\.event\.inputs\.marketing_version \|\| '1\.0\.3'/);
  assert.ok(read('scripts/ci-upload-testflight.sh').includes('MARKETING_VERSION="${MARKETING_VERSION:-1.0.3}"'));
});
