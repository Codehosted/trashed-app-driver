...
test('host clears document/account boundaries and dismisses native sheet before existing Back', () => {
  ...
  assert.match(activity, /handleOnBackPressed\(\) \{\s*if \(nativeNavigation.dismissSheet\(\)\) return;\s*if \(nativeWorkspace != null && nativeWorkspace.back\(\)\) return;\s*if \(nativeChat != null && nativeChat.dismissDialog\(\)\) return;\s*if \(backCheckPending\)/);
