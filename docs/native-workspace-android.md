...
- Per-account/role/permission-scoped data; cancelled/stale request results rejected. NativeWorkspaceCookieStore reads the OS cookie jar for each request URL. Workspace data responses never update that jar; auth/login owns credential changes. Identity changes between initial validation and outgoing headers are rejected. See `android-native-session-safety.md` for current race verification.
