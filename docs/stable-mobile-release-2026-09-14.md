...
- Latest complete checks: **240/240 web tests across 36 files**, final web type gate passes; **99/99 mobile tests**; **7/7 Android functional runtime tests**. Android capture/render disagreement remains a separate unresolved visual gate. Do not interpret functional counts as visual approval or physical-device proof.
...
- User rejected nested purple/white showcase frames. New [full-bleed map concept](../app-store-assets/2026-09/concepts/map-route-showcase-v6.png) includes Danny’s avatar marker, sample chat bubbles and an enlarged route card. A [Trisha companion](../app-store-assets/2026-09/concepts/trisha-mobile-showcase-v4.png) presents messaging, Call Center and Digital Assistant features with lighter typography, an inline messaging icon and branded shirt nametag. Wordmarks remain black; symbols purple; no logo tile. Built-in imagegen prompts/provenance are saved alongside them. These are illustrative concepts, not native screenshots, approved final store exports, sent conversations or live route claims. Artwork only; no app UI changes. Prior drafts remain provenance only.
