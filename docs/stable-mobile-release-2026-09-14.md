...
- User rejected nested purple/white showcase frames. New [full-bleed map concept](../app-store-assets/2026-09/concepts/map-route-showcase-v5.png) includes an enlarged route card and a driver marker with its chat context window open, using existing brand/map/component references. Wordmark remains black; symbol purple; no logo tile. Built-in imagegen prompts/provenance are saved alongside it. This is an 851×1848 illustrative concept with demo content, not a native screenshot, approved final store export, or live route claim. Artwork only; no app UI changes. Prior framed drafts remain provenance only.
