...
**Final Android synthetic runtime:** the same 10-test onboarding/text/byte-export suite passed **10/10 in 171.586 seconds** on the final header/login APK, API36 disposable read-only emulator. It measured the 28dp purple symbol/8dp gap, unframed art, safe bar appearance, 50dp Google/Sign In controls, and retained first-use/UA/permission behavior; file tests verified bounded synthetic bytes, cancellation, concurrency, and cleanup through intercepted intents and a test-only destination provider. No real Files chooser, customer login, order, call, or push was exercised. Exact run, APK hashes, and test-capture provenance: `artifacts/native-experience/onboarding-redesign/android-runtime/`.

The current normal test screenshot for page 1 is complete/readable. Pages 2/3 still omit unchanged header/status content in the capture despite passing view-visibility assertions; their art, body copy, progress, and controls are visible. This does **not** close the separate full Android visual gate. No new capture method, screenshot editing, or production redraw workaround was used. Android resolves theme colors when presenting native intro/login; live OS theme switching while the same native form remains open is not claimed.
