...
import { signInWithPopup } from '../services/firebase'; // Updated import
  ...
  const handleLogin = async () => {
    try {
      ...
      if(err.code === 'auth/configuration-not-found' || err.code === 'auth/api-key-not-valid') {
        setError("Firebase config is missing or invalid. Please update services/firebase.ts");
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="z-10 w-full max-w-md bg-slate-800/50 backdrop-blur-xl border border-slate-700 p-8 rounded-3xl shadow-2xl">
        ...
        <button 
            onClick={handleLogin}
            className="w-full bg-white text-slate-900 font-bold py-4 rounded-xl flex items-center justify-center gap-3 hover:bg-slate-100 transition-all active:scale-95 shadow-xl"
        >
