...
export const LoginScreen: React.FC = () => {
  ...
  const submit = async () => {
    try {
      ...
      setError('Unable to sign in. Check your credentials and network.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: palette.background }]}> 
      ...
      <Pressable onPress={() => navigation.navigate('Reset')}>
        <Text style={{ color: palette.accent, marginTop: 12 }}>Forgot password?</Text>
      </Pressable>
      <Pressable onPress={() => navigation.navigate('Register')}>
        <Text style={{ color: palette.subtleText, marginTop: 20 }}>Need an account? Register</Text>
      </Pressable>
    </View>
  );
};
